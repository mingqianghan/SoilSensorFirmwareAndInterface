...
{
	bool WorkingID_state = false;
	uint16_t len_sum = 0;
	uint16_t RX_count = 0;
	while(!WorkingID_state)
	{
		if(Get_Received_Line(UART_RXBuffer))
		{
			if(UART_RXBuffer[1] == 'F')
			{
				L1.sensorCount = (UART_RXBuffer[3]-'0')*100 + (UART_RXBuffer[4]-'0')*10
						          +(UART_RXBuffer[5]-'0');
			}
			else if (UART_RXBuffer[1] == 'C')
			{
				...
				for (uint8_t i = 0; i < 8; i++)
				{
				    ...
				    L1.sensorIDs[RX_count * 8 + i] = ((UART_RXBuffer[baseIndex] - '0') * 100 +
				                                      (UART_RXBuffer[baseIndex + 1] - '0') * 10 +
				                                      (UART_RXBuffer[baseIndex + 2] - '0'));
				}
				RX_count++;
			}
			else if (UART_RXBuffer[1] == 'P')
			{
				if(len_sum == L1.sensorCount)
				{
					printf("#Z#SensorIDsRecOK\n");
					WorkingID_state = true;
				}
				else
				{
					printf("#I#IDsLenNotMactched#%d#%d#\n", L1.sensorCount, len_sum);
					L1.sensorCount = 0;
					len_sum = 0;
					RX_count = 0;
				}
			}
			else if (UART_RXBuffer[1] == 'H')
			{
				gotoend = true;
				break;
			}
...
void Get_RandnumtoRadio( uint8_t cmd )
{
	...
	while(1)
	{
		if(((HAL_GetTick() - WaitStartTime) > UART_WAIT_TIME))
		{
			gotoend = true;
			break;
		}
